...
WEAR_S2_RESULTSDIR ?= $(RESULTSDIR)/wear_s2
# overrideable plots dir
WEAR_S2_PLOTSDIR ?= $(PLOTSDIR)/wear_s2
# overrideable tikz dir
WEAR_S2_TIKZDIR ?= $(TIKZDIR)/wear_s2


# block recycles to bench? these only make sense for littlefs
WEAR_S2_BLOCK_RECYCLES ?= 0,1,10,100,1000

# number of static files to create
WEAR_S2_STATIC_COUNTS ?= 24
...
## Run benches
.PHONY: all bench bench-wear-s2
all bench: bench-wear-s2
bench-wear-s2: \
		$(foreach c, $(BENCH_CASES), \
			$(foreach fs, $(BENCH_FILESYSTEMS), \
				$(foreach g, $(BENCH_GEOMETRIES), \
					$(WEAR_S2_RESULTSDIR)/bench_wear_s2.$(c).$(fs).$(g).csv)))
...
$1: $($(U_$3)_BENCH_RUNNER)
	$$(strip ./scripts/bench.py -R$$< -B bench_wear_$2 \
		...
		$(if $(filter $3,$\
			...
			-DBLOCK_RECYCLES=$(or $5,$(WEAR_S2_BLOCK_RECYCLES))) \
		-DSTATIC_COUNT=$(or $7,$(WEAR_S2_STATIC_COUNTS)) \
		-o$$@)
endef
...
## Plot benchmarks
.PHONY: all plot plot-wear-s2
all plot: plot-wear-s2
plot-wear-s2: \
		$(WEAR_S2_PLOTSDIR)/plots.html \
		$(foreach g, $(BENCH_GEOMETRIES), \
			$(WEAR_S2_PLOTSDIR)/plot_wear_s2.$(g).svg) \
		$(foreach g, $(BENCH_GEOMETRIES), \
			$(WEAR_S2_PLOTSDIR)/plot_wear_s2_wa.$(g).svg)

## Create a quick html page for easy viewing
$(WEAR_S2_PLOTSDIR)/plots.html:
	echo -e "$(subst $(nl),\n,$(HTML_HEADER))" >> $@
	$(foreach g, $(BENCH_GEOMETRIES), \
		echo -e "<p><img src="plot_wear_s2.$(g).svg"></p>" >> $@ $(nl))
	$(foreach g, $(BENCH_GEOMETRIES), \
		echo -e "<p><img src="plot_wear_s2_wa.$(g).svg"></p>" >> $@ $(nl))
	echo -e "$(subst $(nl),\n,$(HTML_FOOTER))" >> $@
...
# plot rules
$(foreach g, $(BENCH_GEOMETRIES), \
	$(eval $(call PLOT_WEAR_S2_WA_RULE,$\
		$(WEAR_S2_PLOTSDIR)/plot_wear_s2_wa.$(g).svg,$\
		$(foreach c, $(BENCH_CASES),$\
			$(foreach fs, $(BENCH_FILESYSTEMS),$\
				$(WEAR_S2_RESULTSDIR)/bench_wear_s2.$(c).$(fs).$(g).csv)),$\
...
.PHONY: all tikz tikz-wear-s2
all tikz tikz-wear-s2: \
        $(foreach c, $(BENCH_CASES), \
            $(foreach fs, $(BENCH_FILESYSTEMS), \
                $(foreach g, $(BENCH_GEOMETRIES), \
                    $(WEAR_S2_TIKZDIR)/tikz_wear_s2.$(c).$(fs).$(g).csv))) \
        $(foreach c, $(BENCH_CASES), \
            $(foreach fs, $(BENCH_FILESYSTEMS), \
                $(foreach g, $(BENCH_GEOMETRIES), \
                    $(WEAR_S2_TIKZDIR)/tikz_wear_s2_sorted.$(c).$(fs).$(g).csv))) \
        $(foreach c, $(BENCH_CASES), \
            $(foreach fs, $(BENCH_FILESYSTEMS), \
                $(foreach g, $(BENCH_GEOMETRIES), \
                    $(WEAR_S2_TIKZDIR)/tikz_wear_s2_wa.$(c).$(fs).$(g).csv)))
...
$(foreach c, $(BENCH_CASES), \
	$(foreach fs, $(BENCH_FILESYSTEMS), \
		$(foreach g, $(BENCH_GEOMETRIES), \
			$(eval $(call TIKZ_WEAR_S2_RULE,$\
				$(WEAR_S2_TIKZDIR)/tikz_wear_s2.$(c).$(fs).$(g).csv,$\
				$(WEAR_S2_RESULTSDIR)/bench_wear_s2.$(c).$(fs).$(g).csv,$\
				$(WEAR_S2_BLOCK_RECYCLES))))))
...
$(foreach c, $(BENCH_CASES), \
	$(foreach fs, $(BENCH_FILESYSTEMS), \
		$(foreach g, $(BENCH_GEOMETRIES), \
			$(eval $(call TIKZ_WEAR_S2_WA_RULE,$\
				$(WEAR_S2_TIKZDIR)/tikz_wear_s2_wa.$(c).$(fs).$(g).csv,$\
				$(WEAR_S2_RESULTSDIR)/bench_wear_s2.$(c).$(fs).$(g).csv,$\
				$(WEAR_S2_BLOCK_RECYCLES))))))
...
## Save bench results
.PHONY: save save-results save-results-wear-s2
save save-results: save-results-wear-s2
save-results-wear-s2:
	mkdir -p $(SAVEDIR)/$(RESULTSDIR)/
	cp -ru $(WEAR_S2_RESULTSDIR) $(SAVEDIR)/$(RESULTSDIR)/

## Save bench plots
.PHONY: save save-plots save-plots-wear-s2
save save-plots: save-plots-wear-s2
save-plots-wear-s2:
	mkdir -p $(SAVEDIR)/$(PLOTSDIR)/
	cp -ru $(WEAR_S2_PLOTSDIR) $(SAVEDIR)/$(PLOTSDIR)/

## Save tikz
.PHONY: save save-tikz save-tikz-wear-s2
save save-tikz: save-tikz-wear-s2
save-tikz-wear-s2:
	mkdir -p $(SAVEDIR)/$(TIKZDIR)/
	cp -ru $(WEAR_S2_TIKZDIR) $(SAVEDIR)/$(TIKZDIR)/
...
.PHONY: reuse-results touch-results reuse-results-wear-s2 touch-results-wear-s2
reuse-results touch-results: reuse-results-wear-s2 touch-results-wear-s2
reuse-results-wear-s2 touch-results-wear-s2:
	find $(WEAR_S2_RESULTSDIR) -name '*.csv' -execdir touch '{}' ';'
...
## Clean bench results
.PHONY: clean clean-results clean-results-wear-s2
clean clean-results: clean-results-wear-s2
clean-results-wear-s2:
	rm -rf $(WEAR_S2_RESULTSDIR)
	@echo "# note: Not cleaning saved output"

## Clean bench plots
.PHONY: clean clean-plots clean-plots-wear-s2
clean clean-plots: clean-plots-wear-s2
clean-plots-wear-s2:
	rm -rf $(WEAR_S2_PLOTSDIR)
	@echo "# note: Not cleaning saved output"

## Clean tikz
.PHONY: clean clean-tikz clean-tikz-wear-s2
clean clean-tikz: clean-tikz-wear-s2
clean-tikz-wear-s2:
	rm -rf $(WEAR_S2_TIKZDIR)
	@echo "# note: Not cleaning saved output"
